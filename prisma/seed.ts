...
async function main() {
  ...
  for (const mod of ["CRM", "AI_WHATSAPP", "AUTOMATIONS"] as const) {
    await prisma.organizationModule.upsert({
      where: { organizationId_module: { organizationId: demoOrg.id, module: mod } },
      update: {},
      create: { organizationId: demoOrg.id, module: mod, status: "ACTIVE", source: "SUBSCRIBED" },
    });
  }
  ...
  for (const mod of ["CRM", "AI_WHATSAPP", "AUTOMATIONS"] as const) {
    await prisma.organizationModule.upsert({
      where: { organizationId_module: { organizationId: wolfOrg.id, module: mod } },
      update: {},
      create: { organizationId: wolfOrg.id, module: mod, status: "ACTIVE", source: "SUBSCRIBED" },
    });
  }
