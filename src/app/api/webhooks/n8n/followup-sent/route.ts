...
export async function POST(req: NextRequest) {
  ...
  const rateLimit = await checkRateLimit(`webhook:followup-sent:${orgId}`, { limit: 120, windowMs: 60 * 1000 });
  ...
  const result = await ingestWebhookEvent(
    { organizationId: orgId, source: "n8n", eventType: "followup.sent", payload: parsedBody, externalEventId },
    () => processFollowUpSentEvent(parsedBody, orgId)
  );
