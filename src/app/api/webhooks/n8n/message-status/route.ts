...
export async function POST(req: NextRequest) {
  ...
  const rateLimit = await checkRateLimit(`webhook:message-status:${orgId}`, { limit: 120, windowMs: 60 * 1000 });
  ...
  const result = await ingestWebhookEvent(
    { organizationId: orgId, source: "n8n", eventType: "message.status", payload: parsedBody, externalEventId },
    () => processMessageStatusEvent(parsedBody, orgId)
  );
