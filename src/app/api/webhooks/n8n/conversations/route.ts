...
export async function POST(req: NextRequest) {
  ...
  const result = await ingestWebhookEvent(
    { organizationId: orgId, source: "n8n", eventType: "conversation.message", payload: parsedBody, externalEventId },
    () => processConversationEvent(parsedBody, orgId)
  );
  ...
  return NextResponse.json({ success: true, conversationId: result.result.conversationId });
}
