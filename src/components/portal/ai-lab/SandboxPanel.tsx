...
export function SandboxPanel({ prompts, initialSessions }: SandboxPanelProps) {
  ...
  useEffect(() => {
    ...
    getSandboxSession(selectedId)
      ...
      .catch((e) => toast.error(e instanceof Error ? e.message : "Error al cargar la sesión"))
    ...
    startCreating(async () => {
      try {
        ...
        const created: SandboxSessionListItem = {
          id: result.sessionId,
          organizationId: "",
          name: newName.trim() || "Sesión de prueba",
  ...
  function handleDeleteSession(id: string) {
    startTransitionSafe(async () => {
      try {
        ...
        toast.error(e instanceof Error ? e.message : "Error al eliminar la sesión");
      }
    });
  }
  ...
  function handleSend() {
    ...
    startSending(async () => {
      try {
        ...
        toast.error(e instanceof Error ? e.message : "Error al generar la respuesta");
      }
    });
  }
    ...
    <div className="grid grid-cols-3 gap-4">
      <Card className="col-span-1">
        <CardContent className="p-3">
          <div className="mb-3 flex items-center justify-between">
            <p className="text-sm font-semibold text-slate-900">Sesiones de prueba</p>
            <Dialog open={createOpen} onOpenChange={setCreateOpen}>
              <DialogTrigger asChild>
                <Button size="sm" variant="outline"><Plus className="h-4 w-4" />Nueva</Button>
              </DialogTrigger>
              <DialogContent>
                <DialogHeader><DialogTitle>Nueva sesión de sandbox</DialogTitle></DialogHeader>
                <div className="space-y-3">
                  <div className="space-y-1.5">
                    <Label>Nombre (opcional)</Label>
                    <Input value={newName} onChange={(e) => setNewName(e.target.value)} placeholder="Prueba de saludo v2" />
                  </div>
                  <div className="space-y-1.5">
                    <Label>Prompt/versión a probar (opcional)</Label>
                    <Select value={newVersionId} onValueChange={setNewVersionId}>
                      <SelectTrigger><SelectValue placeholder="Usar el prompt SYSTEM activo" /></SelectTrigger>
                      <SelectContent>
                        {options.map((o) => (
          ...
          {sessions.length === 0 ? (
            <EmptyState icon={MessageSquare} title="Sin sesiones" description="Crea una sesión para empezar a probar." />
          ) : (
      ...
      <Card className="col-span-2">
        <CardContent className="flex h-[32rem] flex-col p-3">
          {!selectedSession ? (
            <EmptyState icon={MessageSquare} title="Selecciona o crea una sesión" className="flex-1 justify-center" />
          ) : (
            <>
              <div ref={scrollRef} className="flex-1 space-y-3 overflow-y-auto pr-1">
                {loadingMessages ? (
                  ...
                  <EmptyState icon={MessageSquare} title="Sin mensajes" description="Escribe algo para probar el asistente." />
                ) : (
                  messages.map((m) => (
                          ...
                          <div className="mt-1 flex flex-wrap items-center gap-1">
                            ...
                            {m.knowledgeBaseContext.length > 0 && (
                              <Badge variant="info" className="text-[10px]">
                                ...
                                {m.knowledgeBaseContext.length} fuente(s)
                              </Badge>
                            )}
                          </div>
                        )}
